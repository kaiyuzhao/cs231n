...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # f = lambda x: np.log(1.0 + np.exp(x)) # softplus
    z1 = X.dot(W1) + b1
    f = lambda x: x.clip(0) # rectifier
    a1 = f(z1)
    z2 = a1.dot(W2) + b2
    scores = z2
    #############################################################################
    ...
    loss = jtheta.sum() / float(len(prob))
    theta = np.sum(np.square(W1))
    theta += np.sum(np.square(b1))
    theta += np.sum(np.square(W2))
    theta += np.sum(np.square(b2))
    loss = loss + reg / 2.0 * (theta)
    ...
    # der = lambda x: ( 1 - softplus(x) ) * softplus(x)
